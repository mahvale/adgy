...
useEffect(() => {
  ...
  const fetchEntries = async () => {
    try {
      const entriesRef = collection(dbstorage, 'money_entries');
      let q;

      if (selectedMonth) {
        q = query(entriesRef, where('mois', '==', selectedMonth));
      } else {
        q = query(entriesRef);
      }
...
const handleAddEntry = async () => {
  if (description && amount && date) {
    try {
      const mois = getMonthNameFromDate(date);
      const entriesRef = collection(dbstorage, 'money_entries');
      const newEntry = {
        description,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      <View style={styles.container}>
        <Text style={styles.doneActivities}>Listes des Entrees ({tasks.length})</Text>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              <Text style={styles.modalTitle}>Ajouter une Entrees</Text>
              <TextInput
                style={styles.input}
