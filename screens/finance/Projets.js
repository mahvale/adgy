...
const ProductCard = ({ item }) => {
  ...
  const fetchEntries = async () => {
    try {
      ...
      const entriesCollection = collection(dbstorage, 'Amendes');
  ...
  return (
    <View>
      <View style={{
        ...
        <Text style={{ color: '#fff', marginVertical: 10, fontWeight: 'bold', textAlign: 'center' }}>TABLEAU DE BORD Amendes</Text>
      ...
      <ScrollView style={{ height: '90%', backgroundColor: '#f7f7f7' }} contentContainerStyle={{ flexGrow: 1 }}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between' }} >
          <Text style={{ color: color, fontWeight: 'bold', fontSize: 15 }}>Amendes Mensuel {formatCurrency(total)}</Text>
           <Ionicons name="add-circle" onPress={() => setIsVisible(true)} size={20} color={color} />
        </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={[styles.modalTitle, { color }]}>Ajouter une Amendes</Text>
            <Picker
              selectedValue={name}
              style={styles.input}
